...
void REAgency::generateCatalog()
{
    ...
    {
        PropertyTypeItem pi(properties[i]->getAddress(), properties[i]->getPostalCode(), properties[i]->getTypology(), properties[i]->getPrice());
        addTypeItem(properties[i]);
        pi.setItems(properties);
        catalogItems.insert(pi);
    }
}
